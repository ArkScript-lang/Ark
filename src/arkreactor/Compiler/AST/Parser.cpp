...
namespace Ark::internal
{
    ...
    void Parser::feed(const std::string& code, const std::string& filename)
    {
        ...
        if (filename != ARK_NO_NAME_FILE)
        {
            ...
            spdlog::debug("<Parser> New instance for {}", m_file);
    ...
    std::string Parser::seekFile(const std::string& file)
    {
        ...
        spdlog::debug(
            "<Parser> Importing {}; file: {}; filename: {}; libdir: {}",
            path,
            file,
            Utils::getFilenameFromPath(file),
            std::accumulate(std::next(m_libenv.begin()), m_libenv.end(), m_libenv[0], [](const std::string& a, const std::string& b) -> std::string {
                return a + ", " + b;
            }));
    ...
    void Parser::throwParseError(const std::string& message, internal::Token token)
    {
        ...
        if (m_file != ARK_NO_NAME_FILE)
            ss << "In file " << m_file << "\n";
        ss << makeTokenBasedErrorCtx(token.token, token.line, token.col, m_code);
