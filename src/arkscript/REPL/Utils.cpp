...
namespace Ark::internal
{
    ...
    {
        return static_cast<long>(std::count(line.begin(), line.end(), open)) - static_cast<long>(std::count(line.begin(), line.end(), close));
    }
    ...
    replxx::Replxx::completions_t hookCompletion(const std::string& context, int& length)
    {
        ...
        std::string prefix = context.substr(prefix_len);
    ...
    void hookColor(const std::string& context, replxx::Replxx::colors_t& colors)
    {
        ...
        for (const auto& e : ColorsRegexDict)
        {
            std::size_t pos = 0;
            ...
            while (std::regex_search(str, match, std::regex(e.first)))
            {
                ...
                std::size_t len = codepointLength(c);
